...
/*vot*/	s += '<th><a href="###" onclick="refreshcalendar(yy, mm-1)" title="الشهر الماضي">&laquo;</a></th>';
/*vot*/	s += '<th colspan="5" style="text-align: center"><a href="###" onclick="showdiv(\'year\');_cancelBubble(event)" title="حدد السنة" id="year"></a>&nbsp; - &nbsp;<a id="month" title="حدد الشهر" href="###" onclick="showdiv(\'month\');_cancelBubble(event)"></a></th>';
/*vot*/	s += '<th><A href="###" onclick="refreshcalendar(yy, mm+1)" title="الشهر التالي">&raquo;</A></th>';
	s += '</tr>';
	s += '<tr id="calendar_header"><td>أحد</td><td>اثنين</td><td>ثلاثاء</td><td>اربعاء</td><td>خميس</td><td>جمعة</td><td>سبت</td></tr>';
	s += '</thead>';
	s += '<tbody>';
	for(var i = 0; i < 6; i++) {
		s += '<tr>';
