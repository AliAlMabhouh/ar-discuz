...
function dgmdate($timestamp, $format = 'dt', $timeoffset = '9999', $uformat = '') {
	...
	if($format == 'u') {
		...
		if($timestamp >= $todaytimestamp) {
			if($time > 3600) {
				$return = $lang['before'].intval($time / 3600).'&nbsp;'.$lang['hour']; //Edited by Ar-Discuz.Net 
			} elseif($time > 1800) {
				$return = $lang['before'].$lang['half'].$lang['hour']; //Edited by Ar-Discuz.Net 
			} elseif($time > 60) {
				$return = $lang['before'].intval($time / 60).'&nbsp;'.$lang['min']; //Edited by Ar-Discuz.Net 
			} elseif($time > 0) {
				$return = $lang['before'].$time.'&nbsp;'.$lang['sec']; //Edited by Ar-Discuz.Net 
			} elseif($time == 0) {
				$return = $lang['now'];
			} else {
				$return = $s;
