...
//utility/update.php
	...
	'del_update_lock'		=> 'يجب دخول ال ftp , حذف ملف  ./data/update.lock ثم اعد تحديث الصفحة.',//'请你先登录服务器ftp,手工删除 ./data/update.lock 文件,再次运行本文件进行升级.',
	'sql_file'			=> ' ملف SQL  ',//'SQL文件 '
	'does_not_exist'		=> '  غير موجود ',//' 不存在',
	'table_delete_completed'	=> 'حذف الجداول والحقول تم بنجاح ',//'删除表和字段操作完成了',
	'close_before_update'		=> ' الرجاء أغلق الدخول للمنتدى قبل عملية الترقية . ',//'请先关闭站点再执行此升级操作',
	...
	'check_plugins'			=> 'بعد الترقية بنجاح, أغلق كل الهاكات ليعمل ديسكاز بالشكل الصحيح. بعد ذلك قم بفتح هاك تلو الآخر لمعرفة أن كان هناك خطأ في احداها بأن تقوم بترقيته لتوافق مع إصدار ديسكاز الحالي .',//'升级完成后会关闭所有插件以确保正常运行，请站长逐个开启每一个插件检测是否兼容新版本。',
...
//	'sql_error'			=> ' Error. Please perform the following update statement by hand, then re-run the upgrade program',//'出错,请手工执行以下升级语句后,再重新运行本升级程序',
	...
	'users_updated'			=> 'عملية ترقية أسماء الأعضاء تمت بنجاح',//'用户栏目升级完毕',
	...
	'config_continue'		=> 'بعد إكمال هذه العملية أنقر هنا للإستمرار',//'当你完成上述操作后点击这里继续',
	'data_delete_next'		=> 'تم حذف البيانات بنجاح, الذهاب للخطوة التالية',//'数据删除不处理,进入下一步',
	'tables_unused'			=> '<strong>جداول البيانات التالية</strong> غير مستعملين بعد المقارنة بالقاعدة الاصلية:<br>هل تريد حذفها',//'以下 <strong>数据表</strong> 与标准数据库相比是多余的:<br>你可以根据需要自行决定是否删除',
	'index'				=> 'فهرس',//'索引',
	'field'				=> 'حقل',//'字段',
	'fields_unused'			=> '<strong>الحقول التالية</strong> غير مستعملة بعد مقارنتها بالقاعدة الاصلية:<br>قرر ما ذا كنت تريد حذفهم',//'以下 <strong>字段</strong> 与标准数据库相比是多余的:<br>你可以根据需要自行决定是否删除',
	'tables_fields_no_unused'	=> 'تمت المقارنة مع قاعدة البيانات الاصلية, لا يوجد جداول إضافية أو حقول إضافية.',//'与标准数据库相比,没有需要删除的数据表和字段',
	'click_next_step'		=> 'أنقر للذهاب للخطوة التالية',//'请点击进入下一步',
	'submit_delete'			=> 'تأكيد الحذف',//'提交删除',
	'ignore_unused'			=> 'يمكنك تجاهل الحقول والجداول الاضافية',//'你也可以忽略多余的表和字段',
	'next_step_directly'		=> 'الذهاب للخطوة التالية مباشرة',//'直接进入下一步',
	'default_style_restore_sure'	=> 'هل تريد إستعادة الإستايل الإفتراضي السابق?',//'请确认是否要恢复默认风格？',
	'yes'				=> 'نعم',//'是',
	'no'				=> 'لا',//'否',
	'default_style_restored'	=> 'تم إستعادة الإستايل الإفتراضي, الذهاب للمرحلة الأخرى',//'默认风格已恢复,进入下一步',
	'cache_update'			=> 'ترقية الملفات المؤقتة, الرجاء الرجوع ...',//'缓存更新中，请稍候 ...',
	'database_updated'		=> 'تهانينا, تم ترقية بنية قاعدة البيانات! لأسباب امنية, إحذف هذا الملف.',//'恭喜，数据库结构升级完成！为了数据安全，请删除本文件。',
