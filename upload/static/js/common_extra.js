...
function _checksec(type, idhash, showmsg, recall, modid) {
	...
	x.get('misc.php?mod=sec' + type + '&action=check&inajax=1&modid=' + modid + '&idhash=' + idhash + '&secverify=' + (BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(secverify) : secverify), function(s){
		...
		if(s.substr(0, 7) == 'succeed') {
			...
			if(showmsg) {
				if(type == 'code') {
					showError('خطأ في رمز الأمان, حاول مرة أخرى');
				} else if(type == 'qaa') {
					showError('إجابة السؤال خاطئة , حاول مرة أخرى');
				}
				recall(0);
			}
		}
...
function _copycode(obj) {
	...
	setCopy(BROWSER.ie ? obj.innerText.replace(/\r\n\r\n/g, '\r\n') : obj.textContent, 'تم النسخ للحافظة');
}
...
function _setCopy(text, msg){
	if(BROWSER.ie) {
		...
		if(r) {
			...
			showDialog('<div class="c"><div style="width: 200px; text-align: center;">فشل النسخ, الرجاء حدد "السماح بالوصول"</div></div>', 'alert');
		}
	} else {
		var msg = '<div class="c"><div style="width: 200px; text-align: center; text-decoration:underline;">نسخ للحافظة</div>' +
...
function _showselect(obj, inpid, t, rettype) {
	...
	if(!obj.id) {
		...
		s = '';
		if(!t) {
			s += showselect_row(inpid, 'يوم واحد', 1, 0, rettype);
			s += showselect_row(inpid, 'أسبوع', 7, 0, rettype);
			s += showselect_row(inpid, 'شهر', 30, 0, rettype);
			s += showselect_row(inpid, '3 أشهر', 90, 0, rettype);
			s += showselect_row(inpid, 'مخصص', -2);
		} else {
			if($(t)) {
				...
				s += showselect_row(inpid, 'مخصص', -1);
			} else {
				s += '<a href="javascript:;" onclick="$(\'' + inpid + '\').value = \'0\'">دائم</a>';
				s += showselect_row(inpid, '7 أيام', 7, 1, rettype);
				s += showselect_row(inpid, '14 يوم', 14, 1, rettype);
				s += showselect_row(inpid, 'شهر', 30, 1, rettype);
				s += showselect_row(inpid, '3 أشهر', 90, 1, rettype);
				s += showselect_row(inpid, '6 أشهر', 182, 1, rettype);
				s += showselect_row(inpid, 'عام', 365, 1, rettype);
				s += showselect_row(inpid, 'مخصص', -1);
			}
		}
		$(div.id).innerHTML = s;
...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	if(!menu && !pn) {
		...
		menu.id = menuid;
		if(cover) {
			menu.innerHTML = '<div class="zoominner" id="' + menuid + '_zoomlayer" style="display:none"><p><span class="y"><a id="' + menuid + '_imglink" class="imglink" target="_blank" title="فتح في نافذة جديدة">فتح في نافذة جديدة</a><a id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="الحجم الفعلي">الحجم الفعلي</a>' +
				'<a href="javascript:;" onclick="hideMenu()" class="imgclose" title="إغلاق">إغلاق</a></span>إستخدم عجلة الفأرة للتقريب أو التبعيد</p>' +
				'<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		} else {
			menu.innerHTML = '<div class="popupmenu_popup" id="' + menuid + '_zoomlayer" style="width:auto"><span class="right y"><a href="javascript:;" onclick="hideMenu()" class="flbc" style="width:20px;margin:0 0 2px 0">إغلاق</a></span>إستخدم عجلة الفأرة للتقريب أو التبعيد<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		}
	...
	if(typeof zoomgroup == 'object' && zoomgroup[obj.id] && typeof aimgcount == 'object' && aimgcount[zoomgroup[obj.id]]) {
		...
		if(authorlength > 1) {
			...
			if(authorcurrent !== '') {
				...
				picpage += ' <div id="zimg_prev" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'0 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'0 -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_prev"><strong>السابق</strong></div> ';
				paid = authorcurrent < authorlength - 1 ? authorimgs[authorcurrent + 1] : authorimgs[0];
				picpage += ' <div id="zimg_next" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'100% 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'100% -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_next"><strong>التالي</strong></div> ';
			}
			if(picpage) {
...
function _showColorBox(ctrlid, layer, k, bgcolor) {
	...
	if(!$(ctrlid + '_menu')) {
		...
		var colortexts = ['أسود', 'رصاصي', 'أخضر زيتي غامق', 'أخضر غامق', 'أزرق رمادي غامق', 'سماوي', 'نيلي', 'أخضر غامق', 'أحمر غامق', 'برتقالي غامق', 'زيتي', 'أخضر', 'Teal', 'أزرق', 'كلسي', 'رمادي غامق', 'أحمر', 'رملي', 'أخضر مصفر', 'أخضر بحري', 'زمردي', 'الأزرق الملكي', 'أرجواني', 'رمادي', 'أرجواني محمر', 'برتقالي', 'أصفر', 'برتقالي حمضي', 'أخضر مزرق', 'سماوي عميق', 'أرجواني غامق', 'فضي', 'زهري', 'أصفر فاقع', 'ليموني', 'أخضر شاحب', 'Cang gem Green', 'أزرق مشرق', 'برقوقي', 'أبيض'];		var str = '';
		var str = '';
		for(var i = 0; i < 40; i++) {
...
function _widthauto(obj) {
	var strs = ['التبديل إلى الشاشة الواسعة', 'التبديل إلى الشاشة الضيقة'];
	if($('css_widthauto')) {
...
function _showCreditmenu() {
	if(!$('extcreditmenu_menu')) {
		...
		menu.innerHTML = '<div class="p_opt"><img src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" /> يرجى الإنتظار...</div>';
		$('append_parent').appendChild(menu);
...
function _setShortcut() {
	$('shortcuttip').onclick = function() {
		var msg = '1-أنقر فوق"' + '<a href="javascript:;" class="xi2 xw1" ';
		msg += 'onclick="this.href = \'forum.php?mod=misc&action=shortcut\';this.click();saveUserdata(\'setshortcut\', 1);"';
		msg += '>تحميل إختصار سطح المكتب</a>' + '"，بعد إكتمال التحميل يمكنك نقل الملفات إلى سطح المكتب<br />';
		msg += '2-أنقر فوق"' + '<a href="forum.php?mod=misc&action=shortcut&type=ico" class="xi2 xw1">';
		msg += 'تحميل الأيقونة</a>' + '"，بعد اكتمال التحميل انقر بزر الماوس الأيمن فوق الملف اختصار على سطح المكتب -> خصائص -> تغيير الرمز، حدد الأيقونة التي تم تحميلها';
		showDialog(msg, 'notice', 'إضافة إختصار على سطح المكتب');
	};
