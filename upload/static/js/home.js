...
function blogAddOption(sid, aid) {
	...
	if (newOption!=null && newOption!='') {
		...
		alert('لا يمكن أن يكون إسم الفئة فارغا!');
		return false;
	}
}
		...
		theImages[i].onload = function() {
			if (this.width > size) {
				...
				if (this.parentNode.tagName.toLowerCase() != 'a') {
					...
					this.title = 'إضغط على الصورة لعرضها بحجمها الطبيعي';
...
function ischeck(id, prefix) {
	...
	for(var i = 0; i < form.elements.length; i++) {
		...
		if(e.name.match(prefix) && e.checked) {
			if(confirm("هل أنت متأكد أنك تريد تنفيذ هذه العملية؟")) {
				return true;
			} else {
				return false;
			}
	...
	alert('الرجاء إختيار العملية التي تريد تنفيذها');
	return false;
}
...
function showFlash(host, flashvar, obj, shareid) {
	...
	var videoMp3 = true;
	if('' == flashvar) {
		alert('رابط ملف الموسيقى خاطئ , لا يمكن تركه فارغا');
		return false;
	}
	if('music' == host) {
	...
	if(flashAddr[host]) {
		...
		var nodetxt = document.createTextNode("أقل");
...
function myfriend_post(uid) {
	if($('friend_'+uid)) {
		$('friend_'+uid).innerHTML = '<p>أنتم أصدقاء , يمكنك أيضا <a href="home.php?mod=space&do=wall&uid='+uid+'" class="xi2" target="_blank">إرسال رسالة</a> أو <a href="home.php?mod=spacecp&ac=poke&op=send&uid='+uid+'&handlekey=propokehk_'+uid+'" id="a_poke_'+uid+'" class="xi2" onclick="showWindow(this.id, this.href, \'get\', 0, {\'ctrlid\':this.id,\'pos\':\'13\'});">إرسال تحية</a></p>';
	}
	showCreditPrompt();
}
...
function feed_more_show(feedid) {
	...
	$(opid).innerHTML = '&laquo; تقليص القائمة';
	$(opid).onclick = function() {
...
function feed_more_close(feedid) {
	...
	$(opid).innerHTML = '&raquo; أكثر نشاطا';
	$(opid).onclick = function() {
