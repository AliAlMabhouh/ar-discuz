...
drag.extend({
	...
	init : function (sampleMode) {
		this.initCommon();
		$('samplepanel').innerHTML = 'الإدارة المباشرة لمحتوى البلوك [<a href="javascript:;" onclick="spaceDiy.cancel();return false;" class="xi2">إلغاء</a>]';
	...
	blockForceUpdate : function (e,all) {
		...
		x.get('portal.php?mod=portalcp&ac=block&op=getblock&forceupdate=1&inajax=1&bid='+bid+'&tpl='+document.diyform.template.value, function(s) {
			if(s.indexOf('errorhandle_') != -1) {
				...
				runslideshow();
				showDialog('عذراً لا تمتلك حقوق التعديل على البلوكات', 'alert');
				doane();
			} else {
