...
(function(){
	...
	Drag.prototype = {
		...
		checkEdit : function (ele) {
			...
			if (!$(id+'_edit')) {
				...
				dom.id = id+'_edit';
				dom.innerHTML = '<span id="'+id+'_edit_menu">تعديل</span>';
				ele.appendChild(dom);
	...
	DIY.prototype = {
		...
		cancel : function () {
			var flag = false;
			if (this.isChange) {
				flag = confirm(this.cancelConfirm ? this.cancelConfirm : 'سيتم فقدان كافة التغييرات عند الاغلاق. هل أنت متأكد من الإغلاق?');
			}
