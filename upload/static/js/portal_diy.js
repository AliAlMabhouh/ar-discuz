...
drag.extend({
	'getBlocksTimer' : '',
	'blocks' : [],
	'blockDefaultClass' : [{'key':'تحديد ستايل','value':''},{'key':'بدون حدود, وهامش','value':'cl_block_bm'},{'key':'ستايل 1','value':'xbs_1'},{'key':'ستايل 2','value':'xbs xbs_2'},{'key':'ستايل 3','value':'xbs xbs_3'},{'key':'ستايل 4','value':'xbs xbs_4'},{'key':'ستايل 5','value':'xbs xbs_5'},{'key':'ستايل 6','value':'xbs xbs_6'},{'key':'ستايل 7','value':'xbs xbs_7'}],
	'frameDefaultClass' : [{'key':'تحديد ستايل','value':''},{'key':'بدون حدود, وهامش','value':'cl_frame_bm'},{'key':'بدون حدود','value':'xfs xfs_nbd'},{'key':'ستايل 1','value':'xfs xfs_1'},{'key':'ستايل 2','value':'xfs xfs_2'},{'key':'ستايل 3','value':'xfs xfs_3'},{'key':'ستايل 4','value':'xfs xfs_4'},{'key':'ستايل 5','value':'xfs xfs_5'}],
	setDefalutMenu : function () {
		this.addMenu('default','العنوان','drag.openTitleEdit(event)');
		this.addMenu('default','ستايل','drag.openStyleEdit(event)');
		this.addMenu('default', 'حذف', 'drag.removeBlock(event)');
		this.addMenu('block', 'البلوك', 'drag.openBlockEdit(event)');
		this.addMenu('block', 'البيانات', 'drag.openBlockEdit(event,"data")');
		this.addMenu('block', 'تحديث', 'drag.blockForceUpdate(event)');
		this.addMenu('frame', 'تصدير', 'drag.frameExport(event)');
		this.addMenu('tab', 'تصدير', 'drag.frameExport(event)');
	},
	setSampleMenu : function () {
		this.addMenu('block', 'السمة', 'drag.openBlockEdit(event)');
		this.addMenu('block', 'البيانات', 'drag.openBlockEdit(event,"data")');
		this.addMenu('block', 'تحديث', 'drag.blockForceUpdate(event)');
	},
	...
	openStyleEdit : function (e) {
		...
		var repeatarr = [{'key':'العنوان','value':'repeat'},{'key':'بدون تكرار','value':'no-repeat'},{'key':'تكرار افقياً','value':'repeat-x'},{'key':'تكرار رأسياً','value':'repeat-y'}];
		var stylearr = [{'key':'بدون ستايل','value':'none'},{'key':'خط متصل','value':'solid'},{'key':'خط منقط','value':'dotted'},{'key':'خط متقطع','value':'dashed'}];
		var table = '<table class="tfm">';
		table += '<tr><th>الخط</th><td><input type="text" id="fontsize" class="px p_fre vm" value="'+fontsize+'" size="2" />px <input type="text" id="fontcolor" class="px p_fre vm" value="'+fontcolor+'" size="2" />';
		table += getColorPalette(id+'_fontPalette', 'fontcolor' ,fontcolor)+'</td></tr>';
		table += '<tr><th>أدخل النص المرتبط</th><td><input type="text" id="linkfontsize" class="px p_fre vm" value="'+linkfontsize+'" size="2" />px <input type="text" id="linkcolor" class="px p_fre vm" value="'+linkcolor+'" size="2" />';
		...
		table += '<tr><th>الحدود</th><td><ul id="borderul" class="'+ulclass+'">';
		table += '<li><label>أعلى</label><select class="ps vm" id="bdtwidth" ><option value="">الحجم</option>'+this.getOption(widtharr,bdtwidth)+'</select>';
		table += ' <select class="ps vm" id="bdtstyle" ><option value="">ستايل</option>'+this.getOption(stylearr,bdtstyle)+'</select>';
		table += ' اللون <input type="text" id="bdtcolor" class="px p_fre vm" value="'+bdtcolor+'" size="7" />';
		table += getColorPalette(id+'_bdtPalette', 'bdtcolor' ,bdtcolor)+'</li>';

		table += '<li class="bordera mtn"><label>يسار</label><select class="ps vm" id="bdrwidth" ><option value="">الحجم</option>'+this.getOption(widtharr,bdrwidth)+'</select>';
		table += ' <select class="ps vm" id="bdrstyle" ><option value="">ستايل</option>'+this.getOption(stylearr,bdrstyle)+'</select>';
		table += ' اللون <input type="text" id="bdrcolor" class="px p_fre vm" value="'+bdrcolor+'" size="7" />';
		table += getColorPalette(id+'_bdrPalette', 'bdrcolor' ,bdrcolor)+'</li>';

		table += '<li class="bordera mtn"><label>أسفل</label><select class="ps vm" id="bdbwidth" ><option value="">الحجم</option>'+this.getOption(widtharr,bdbwidth)+'</select>';
		table += ' <select class="ps vm" id="bdbstyle" ><option value="">ستايل</option>'+this.getOption(stylearr,bdbstyle)+'</select>';
		table += ' اللون <input type="text" id="bdbcolor" class="px p_fre vm" value="'+bdbcolor+'" size="7" />';
		table += getColorPalette(id+'_bdbPalette', 'bdbcolor' ,bdbcolor)+'</li>';

		table += '<li class="bordera mtn"><label>يمين</label><select class="ps vm" id="bdlwidth" ><option value="">الحجم</option>'+this.getOption(widtharr,bdlwidth)+'</select>';
		table += ' <select class="ps vm" id="bdlstyle" ><option value="">ستايل</option>'+this.getOption(stylearr,bdlstyle)+'</select>';
		table += ' اللون <input type="text" id="bdlcolor" class="px p_fre vm" value="'+bdlcolor+'" size="7" />';
		table += getColorPalette(id+'_bdlPalette', 'bdlcolor' ,bdlcolor)+'</li>';
		table += '</ul><p class="ptm"><label><input id="borderop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'borderul\').className = $(\'borderul\').className == \'borderul\' ? \'borderula\' : \'borderul\'">تم تعيين</label></p></td></tr>';
		...
		table += '<tr><th>هامش</th><td><div id="margindiv" class="'+ulclass+'"><span><label>أعلى</label> <input type="text" id="margint" class="px p_fre vm" value="'+margint+'" size="1"/>px </span>';
		table += '<span class="bordera"><label>يسار</label> <input type="text" id="marginr" class="px p_fre vm" value="'+marginr+'" size="1" />px </span>';
		table += '<span class="bordera"><label>أسفل</label> <input type="text" id="marginb" class="px p_fre vm" value="'+marginb+'" size="1" />px </span>';
		table += '<span class="bordera"><label>يمين</label> <input type="text" id="marginl" class="px p_fre vm" value="'+marginl+'" size="1" />px</span>';
		table += '</div><p class="ptm"><label><input id="marginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'margindiv\').className = $(\'margindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'">تم التعيين</label></p></td></tr>';
		...
		if (objType == 1) {
			...
			table += '<tr><th>الحشوة</th><td><div id="cmargindiv" class="'+ulclass+'"><span><label>أعلى</label> <input class="px p_fre" id="cmargint" value="'+cmargint+'" size="1" />px </span>';
			table += '<span class="bordera"><label>يسار</label> <input class="px p_fre" id="cmarginr" value="'+cmarginr+'" size="1" />px </span>';
			table += '<span class="bordera"><label>أسفل</label> <input class="px p_fre" id="cmarginb" value="'+cmarginb+'" size="1" />px </span>';
			table += '<span class="bordera"><label>يمين</label> <input class="px p_fre" id="cmarginl" value="'+cmarginl+'" size="1" />px </span>';
			table += '</div><p class="ptm"><label><input id="cmarginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'cmargindiv\').className = $(\'cmargindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'"> تم التعيين</label></p></td></tr>';
		}
		table += '<tr><th>لون الخلفية</th><td><input type="text" id="bgcolor" class="px p_fre vm" value="'+bgcolor+'" size="4" />';
		table += getColorPalette(id+'_bgcPalette', 'bgcolor' ,bgcolor)+'</td></tr>';
		table += '<tr><th>صورة الخلفية</th><td><input type="text" id="bgimage" class="px p_fre vm" value="'+bgimage+'" size="25" /> <select class="ps vm" id="bgrepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		var classarr = objType == 1 ? this.blockDefaultClass : this.frameDefaultClass;
		table += '<tr><th>الطبقة المحددة</th><td><input type="text" id="diyClassName" class="px p_fre" value="'+diyClassName+'" size="8" /> <select class="ps vm" id="bgrepeat" onchange="$(\'diyClassName\').value=this.value;" >'+this.getOption(classarr, diyClassName)+'</select></td></tr>';
		table += '</table>';

		var wname = objType ? 'الكتلة' : 'الإطار';
		html = '<div class="c diywin" style="width:450px;position:relative;">'+table+'</div>';
		var h = '<h3 class="flb"><em>تعديل'+wname+'ستايل</em><span><a href="javascript:;" class="flbc" onclick="drag.closeStyleEdit(\''+id+'\');return false;" title="إغلاق">\n\
			إغلاق</a></span></h3>';
		var f = '<p class="o pns"><button onclick="drag.saveStyle(\''+id+'\');drag.closeStyleEdit(\''+id+'\');" class="pn pnc" value="true">\n\
			<strong>موافق</strong></button><button onclick="drag.closeStyleEdit(\''+id+'\')" class="pn" value="true"><strong>إلغاء</strong></button></p>';
		this.removeMenu(e);
	...
	openTitleEdit : function (e) {
		...
		var titlename = obj instanceof Block ? ' كتلة ' : 'الإطار';
		var repeatarr = [{'key':'تكرار','value':'repeat'},{'key':'بدون تكرار','value':'no-repeat'},{'key':'تكرار افقياً','value':'repeat-x'},{'key':'تكرار رأسياً','value':'repeat-y'}];
		...
		common += '<tr><th>صورة الخلفية:</th><td><input type="text" id="titleBgImage" class="px p_fre" value="'+bgimage+'" /> <select class="ps vm" id="titleBgRepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		common += '<tr><th>لون الخلفية:</th><td><input type="text" id="titleBgColor" class="px p_fre" value="'+bgcolor+'" size="7" />';
		common += getColorPalette(fid+'bgPalette_0', 'titleBgColor' ,bgcolor)+'</td></tr>';
		if (obj instanceof Tab) {
			var switchArr = [{'key':'عند الضغط','value':'click'},{'key':'عند المرور بالماوس','value':'mouseover'}];
			var switchType = obj.titles['switchType'] ? obj.titles['switchType'][0] : 'click';
			common += '<tr><th>نوع التبديل:</th><td><select class="ps" id="switchType" >'+this.getOption(switchArr,switchType)+'</select></td></tr>';
		}
		common += '</table><hr class="l">';
		var li = '';
		li += '<div id="titleInput_0"><table class="tfm"><tr><th>'+titlename+'العنوان:</th><td><input type="text" id="titleText_0" class="px p_fre" value="`title`" /></td></tr>';
		li += '<tr><th>وصلة:</th><td><input type="text" id="titleLink_0" class="px p_fre" value="`link`" /></td></tr>';
		li += '<tr><th>الصور:</th><td><input type="text" id="titleSrc_0" class="px p_fre" value="`src`" /></td></tr>';
		li += '<tr><th>الموقع:</th><td><select id="titleFloat_0" class="ps vm"><option value="" `left`>إلى اليمين</option><option value="right" `right`>إلى اليسار</option></select>';
		li += '&nbsp;&nbsp;الازاحة: <input type="text" id="titleMargin_0" class="px p_fre vm" value="`margin`" size="2" />px</td></tr>';
		li += '<tr><th>الخط:</th><td><select class="ps vm" id="titleSize_0" ><option value="">الحجم</option>`size`</select>';
		li += '&nbsp;&nbsp;اللون: <input type="text" id="titleColor_0" class="px p_fre vm" value="`color`" size="4" />';
		...
		var c = len + 1;
			html = '<div class="c diywin" style="width:450px;height:400px; overflow:auto;"><table cellspacing="0" cellpadding="0" class="tfm pns"><tr><th></th><td><button type="button" id="addTitleInput" class="pn" onclick="drag.addTitleInput('+c+');"><em>إضافة عنوان جديد</em></button></td></tr></table><div id="titleEdit">'+html+common+'</div></div>';
		var h = '<h3 class="flb"><em> تعديل '+titlename+' العنوان</em><span><a href="javascript:;" class="flbc" onclick="drag.closeTitleEdit(\''+fid+'\');return false;" title="إغلاق">\n\
			إغلاق</a></span></h3>';
		var f = '<p class="o pns"><button onclick="drag.saveTitleEdit(\''+fid+'\');drag.closeTitleEdit(\''+fid+'\');" class="pn pnc" value="true">\n\
			<strong>موافق</strong></button><button onclick="drag.closeTitleEdit(\''+fid+'\')" class="pn" value="true"><strong>إلغاء</strong></button></p>';
		this.removeMenu(e);
	...
	removeBlock : function (e, flag) {
		...
		var obj = this.getObjByName(id);
		if (!flag) {
			if (!confirm('هل أنت متأكد من الحذف؟ لا يمكنك التراجع!')) return false;
		}
		if (obj instanceof Block) {
			this.delBlock(id);
		} else if (obj instanceof Frame) {
	...
	blockForceUpdate : function (e,all) {
		...
		bcontent.innerHTML = '<center>تحميل المحتوى...</center>';
		var x = new Ajax();
		x.get('portal.php?mod=portalcp&ac=block&op=getblock&forceupdate=1&inajax=1&bid='+bid+'&tpl='+document.diyform.template.value, function(s) {
			if(s.indexOf('errorhandle_') != -1) {
				...
				runslideshow();
				showDialog('عذراً لا يوجد لك صلاحيات بتعديل او اضافة بلوكات', 'alert');
				doane();
			} else {
	...
	frameExport : function (e) {
		var flag = true;
		if (drag.isChange) {
			flag = confirm('لقد قمت بعمل بعض التغييرات, الرجاء الاستيراد بعد الحفظ , اذ لم تفعل ذلك سوفت تذهب هذه التغييرات !');
		}
		if (flag) {
			if ( typeof e == 'object') {
				e = Util.event(e);
	...
	endBlockForceUpdateBatch : function () {
		if($('allupdate')) {
			$('allupdate').innerHTML = 'تم إكتمال العملية';
	...
	getBlocks : function () {
		...
		if (this.blocks.length > 0) {
			...
			if($('allupdate')) {
				$('allupdate').innerHTML = 'الإجمالي <span style="color:blue">'+this.blocksLen+'</span> كتلة ,تحديث #<span style="color:red">'+cur+'</span> وحدة ,تم بنجاح<span style="color:red">'+(parseInt(cur / this.blocksLen * 100)) + '%</span>';
	...
	blockForceUpdateBatch : function (blocks) {
		...
		showDialog('<div id="allupdate" style="width:350px;line-height:28px;">بدء التحديث......</div>','confirm','تحديث بيانات الكتل', '', true, 'drag.endBlockForceUpdateBatch()');
		var wait = function() {
	...
	clearAll : function () {
		if (confirm('هل أنت متأكد من أنك تريد حذف كافة الكتل الموجودة في هذه الصفحة بواسطة DIY ? لا يمكن التراجع على هذا القرار.')) {
	...
	createObj : function (e,objType,contentType) {
		if (objType == 'block' && !this.checkHasFrame()) {alert("تحذير: لا يوجد اطار, قم بإضافة اطار.");spaceDiy.getdiy('frame');return false;}
		e = Util.event(e);
	...
	getFrameHtml : function (type) {
		...
		str += '<div id="'+id+'_title" class="'+this.titleClass+' '+this.frameTitleClass+'"><span class="'+this.titleTextClass+'">'+type+'الإطار</span></div>';
		var cols = type.split('-');
		var clsl='',clsc='',clsr='';
		clsl = ' frame-'+type+'-l';
		clsc = ' frame-'+type+'-c';
		...
		if(this.sampleMode) {
			...
			if (!this.isChange) {
				window.onbeforeunload = function() {
					return 'لقد قمت بالتعديل. اذا قمت بالاغلاق, سيتم فقدان كافة التغيرات.';
				};
			}
			this.isChange = true;
	...
	goonDIY : function () {
		if ($('prefile').value == '1') {
			showDialog('<div style="line-height:28px;">إضغط على "استمرار" لجلب المعلومات الحالية للاستايل الحالي,<br />إضغط على زر  "حذف" لحذف البيانات المؤقتة.</div>','confirm','مواصلة إلى  DIY مع البيانات المؤقتة?', function(){location.replace(location.href+'&preview=yes');}, true, 'spaceDiy.cancelDIY()', '', 'مواصلة', 'حذف');
...
spaceDiy.extend({
		...
		if (!optype) {
			if (['portal/portal_topic_content', 'portal/list', 'portal/view'].indexOf(tplpre[0]) == -1) {
				if (document.diyform.template.value.indexOf(':') > -1 && !document.selectsave) {
					...
					showDialog('<form name="selectsave" action="" method="get"><label><input type="radio" value="0" name="savemod"'+schecked+' />تطبيق على كل الصفحات</label>\n\
					<label><input type="radio" value="1" name="savemod"'+dchecked+' />تطبيق فقط على هذه الصفحة</label></form>','notice', '', spaceDiy.save);
					return false;
				}
	...
	cancel : function () {
		...
		if (drag.isClearClose) {
			showDialog('<div style="line-height:28px;">حفظ البيانات المؤقتة?<br /> إضغط موافق للحفظ, إضغط على إلغاء لحذف البيانات المؤقتة </div>','confirm','حفظ البيانات المؤقتة', function(){location.href = spaceDiy.cancelDiyUrl();}, true, function(){window.onunload=function(){spaceDiy.cancelDIY()};location.href = spaceDiy.cancelDiyUrl();});
		} else {
	...
	recover : function() {
		if (confirm('هل أنت متأكد أنك تريد العودة إلى الإصدار السابق من حفظ النتائج?')) {
			drag.clearClose();
