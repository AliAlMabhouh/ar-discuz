...
/*vot*/	s += '<tr align="center" id="calendar_week"><td><a href="javascript:;" onclick="refreshcalendar(yy, mm-1)" title="الشهر السابق">&laquo;</a></td><td colspan="5" style="text-align: center"><a href="javascript:;" onclick="showdiv(\'year\');doane(event)" class="dropmenu" title="العام القادم" id="year"></a>&nbsp; - &nbsp;<a id="month" class="dropmenu" title="حدد الشهر" href="javascript:;" onclick="showdiv(\'month\');doane(event)"></a></td><td><a href="javascript:;" onclick="refreshcalendar(yy, mm+1)" title="الشهر القادم">&raquo;</a></td></tr>';
/*vot*/	s += '<tr id="calendar_header"><td>السبت</td><td>الأحد</td><td>الإثنين</td><td>الثلاثاء</td><td>الأربعاء</td><td>الخميس</td><td>الجمعة</td></tr>';
	for(var i = 0; i < 6; i++) {
		s += '<tr>';
		for(var j = 1; j <= 7; j++)
...
/*vot*/	s += '<tr id="hourminute" class="pns"><td colspan="4" align="left"><input type="text" size="1" value="" id="hour" class="px vm" onKeyUp=\'this.value=this.value > 23 ? 23 : zerofill(this.value);controlid.value=controlid.value.replace(/\\d+(\:\\d+)/ig, this.value+"$1")\'> ساعات';
/*vot*/	s += '<span id="fullhourselector"><input type="text" size="1" value="" id="minute" class="px vm" onKeyUp=\'this.value=this.value > 59 ? 59 : zerofill(this.value);controlid.value=controlid.value.replace(/(\\d+\:)\\d+/ig, "$1"+this.value)\'> دقائق </span>';
/*vot*/	s += '<span id="halfhourselector"><select id="minutehalfhourly" onchange=\'this.value=this.value > 59 ? 59 : zerofill(this.value);controlid.value=controlid.value.replace(/(\\d+\:)\\d+/ig, "$1"+this.value)\'><option value="00">00</option><option value="30">30</option></select> نصف ساعة </span>';
/*vot*/	s += '</td><td align="right" colspan="3"><button class="pn" onclick="confirmcalendar();"><em>موافق</em></button></td></tr>';
	s += '</table></div></div>';
	...
	for(var k = 1; k <= 12; k++) {
/*vot*/		s += '<a href="javascript:;" onclick="refreshcalendar(yy, ' + (k - 1) + ');$(\'calendar_month\').style.display=\'none\'"><span' + (today.getMonth()+1 == k ? ' class="calendar_today"' : '') + ' id="calendar_month_' + k + '">' + k + ( k < 10 ? '&nbsp;' : '') + ' الشهر </span></a><br />';
	}
	s += '</div>';
