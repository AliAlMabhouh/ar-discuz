...
function fileQueued(file) {
	try {
		...
		if(createQueue) {
			progress.setStatus("الرجاء الانتظار...");
		} else {
...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			...
			showDialog("لقد قمت بإختيار ملفات كثيرة\n"+(message === 0 ? " لا يمكنك إضافة ملفات أخرى " : "يمكنك إضافة فقط " + message + " ملف"), 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			return;
		}
		...
		switch (errorCode) {
			case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
				progress.setStatus("الملف كبير جداً.");
				this.debug("Error Code: File too big, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
				progress.setStatus("الملف فارغ.");
				this.debug("Error Code: Zero byte file, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
				progress.setStatus("هذا النوع من الملفات غير مدعوم.");
				this.debug("Error Code: Invalid File Type, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
				alert("لقد قمت بإختيار ملفات كثيرة جداً.  " +  (message > 1 ? "يمكنك إضافة فقط " +  message + " ملف" : "لا يمكنك اضافة اي ملفات أخرى."));
				break;
			default:
				if (file !== null) {
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	...
	try {
		...
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setStatus(" الإجمالي ("+percent+"%)...");
	...
	try {
		...
		if(this.customSettings.uploadSource == 'forum') {
			...
			if(parseInt(data.picid)) {
				...
				newTd = document.createElement("TD");
				newTd.className = 'd';
				newTd.innerHTML = 'وصف الملف <br/><textarea name="title['+data.picid+']" cols="40" rows="2" class="pt"></textarea>';
				newTr.appendChild(newTd);
				this.customSettings.imgBoxObj.appendChild(newTr);
			} else {
				showDialog('فشل رفع الصورة', 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			}
			$(file.id).style.display = 'none';
		} else if(this.customSettings.uploadType == 'blog') {
			var data = eval('('+serverData+')');
			if(parseInt(data.picid)) {
				...
				showDialog('فشل الرفع', 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			}
			$(file.id).style.display = 'none';
		} else if(this.customSettings.uploadSource == 'portal') {
			var data = eval('('+serverData+')');
			...
			progress.setStatus("تم إكتمال الرفع بحمد الله.");
			progress.toggleCancel(false);
		}
	} catch (ex) {
		this.debug(ex);
	...
	try {
		...
		switch (errorCode) {
			case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
				progress.setStatus("خطأ الرفع: " + message);
				this.debug("Error Code: HTTP Error, File name: " + file.name + ", Message: " + message);
				break;
			case SWFUpload.UPLOAD_ERROR.MISSING_UPLOAD_URL:
				progress.setStatus("فشل التكوين");
				this.debug("Error Code: No backend file, File name: " + file.name + ", Message: " + message);
				break;
			case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
				progress.setStatus("فشل الرفع.");
			...
			case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
				progress.setStatus("خطأ أمني");
				this.debug("Error Code: Security Error, File name: " + file.name + ", Message: " + message);
				break;
			case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
				progress.setStatus("تجاوزت الحد الأقصى.");
				this.debug("Error Code: Upload Limit Exceeded, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND:
				progress.setStatus("الملف غير موجود.");
			...
			case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
				...
				progress.setStatus(this.customSettings.alertType ? STATUSMSG[this.customSettings.alertType] : "تم الإلغاء");
				progress.setCancelled();
				break;
			case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
				progress.setStatus("تم الايقاف");
				break;
			default:
