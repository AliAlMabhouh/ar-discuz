...
function insertAttach(id) {
	...
	if(typeof no_insert=='undefined') {
		localfile += '&nbsp;<a href="javascript:;" class="xi2" title="إضغط هنا للإدراج في مكان المؤشر" onclick="insertAttachimgTag(' + id + ');return false;">[إدراج]</a>';
	}
	inhtml += '<td><strong>' + localfile +'</strong>';
	inhtml += '</td><td class="d">وصف الصورة<br/><textarea name="pic_title" cols="40" rows="2" class="pt"></textarea>';
	inhtml += '</td><td class="o"><span id="showmsg' + id + '"><a href="javascript:;" onclick="delAttach(' + id + ');return false;" class="xi2">[حذف]</a></span>';
	inhtml += '</td></tr></table>';
...
function upload() {
	...
	if(nowUid>0) {
		...
		if(uploadStat==1) {
			upobj.innerHTML = ' تم الرفع بنجاح ';
			successState = true;
			var InputNode;
			try {
			...
			upobj.innerHTML = 'فشل الرفع '+uploadStat;
		}
	}

	if($('showmsg'+nid) != null) {
		$('showmsg'+nid).innerHTML = 'جاري الرفع, الرجاء الإنتظار (<a href="javascript:;" onclick="forms[nowUid].submit();">إعادة المحاولة</a>)';
