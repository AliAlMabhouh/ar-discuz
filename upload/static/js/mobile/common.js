	...
	converthtml : function() {
		...
		if(lastpage) {
			...
			for(var i=1; i<=lastpage; i++) {
				selector += '<option value="'+i+'" '+ (i == curpage ? 'selected' : '') +'>الصفحة '+i+'</option>';
			}
			selector += '</select>';
			selector += '<span>الصفحة'+curpage+'</span>';
		}

		$('div.pg').removeClass('pg').addClass('page').html('<a href="'+ prevpagehref +'">السابق</a>'+ selector +'<a href="'+ nextpagehref +'">التالي</a>');
...
var img = {
	...
	errorhandle : function(obj, is_err_t) {
		...
		if(is_err_t) {
			var parentnode = obj.parent();
			parentnode.find('.loading').remove();
			parentnode.append('<div class="error_text">أنقر لإعادة التحميل</div>');
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			$('#ntcmsg').remove();
			if(type == 'alert') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="button2" type="button" value="تحديد" onclick="popup.close();"></dd></div>'
			} else if(type == 'confirm') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="redirect button2" type="button" value="تحديد" href="'+ url +'"><a href="javascript:;" onclick="popup.close();">إلغاء</a></dd></div>'
			}
...
var geo = {
	...
	locationerror : function(error) {
		...
		switch(error.code) {
			case error.TIMEOUT:
				geo.errmsg = "إنتهى وقت الحصول على الموقع , حاول لاحقا";
				break;
			case error.POSITION_UNAVAILABLE:
				geo.errmsg = 'لا يمكن الكشف عن موقعك الحالي';
			    break;
		    case error.PERMISSION_DENIED:
		        geo.errmsg = 'يرجى السماح بالوصول إلى موقعك';
		        break;
		    case error.UNKNOWN_ERROR:
		        geo.errmsg = 'حدث خطأ غير معروف';
		        break;
		}
	},
	locationsuccess : function(position) {
		...
		$.ajax({
			type:'POST',
			url:'http://maps.google.com/maps/api/geocode/json?latlng=' + geo.latitude + ',' + geo.longitude + '&language=ar-AR&sensor=true', //KHALID//
			dataType:'json'
		})
		.success(function(s) {
			if(s.status == 'OK') {
	...
	init : function() {
		...
		.on('touchmove', function(e) {
			...
			if(!status && $(window).scrollTop() <= 0) {
				...
				contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_arrow.gif" style="vertical-align:middle;margin-right:5px;-moz-transform:rotate(180deg);-webkit-transform:rotate(180deg);-o-transform:rotate(180deg);transform:rotate(180deg);"><span id="refreshtxt">وصول للتحديث</span>');
			...
			if(status == true) {
				...
				if(pullheight >= 0 && pullheight < 150) {
					...
					if(reloadflag) {
						...
						contentobj.find('#refreshtxt').html('وصول للتحديث');
					}
					reloadflag = false;
					...
					if(!reloadflag) {
						...
						contentobj.find('#refreshtxt').html('تخفيف التحديث');
					}
					reloadflag = true;
				}
			}
		...
		.on('touchend', function(e) {
			if(status == true) {
				if(reloadflag) {
					contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_load.gif" style="vertical-align:middle;margin-right:5px;">جاري التحميل ...');
