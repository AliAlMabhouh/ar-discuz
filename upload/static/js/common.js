...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	...
	if(locationtime) {
		leftmsg = locationtime + ' ثانية متبقية ليتم تحويلك للصفحة المطلوب';
		showDialogST = setTimeout(closefunc, locationtime * 1000);
		showconfirm = 0;
	}
	confirmtxt = confirmtxt ? confirmtxt : confirmtxtdefault;
	canceltxt = canceltxt ? canceltxt : 'إلغاء';
	...
	s += t ? t : 'تذكي';
	s += '</em><span><a href="javascript:;" id="fwin_dialog_close" class="flbc" onclick="hideMenu(\'' + menuid + '\', \'dialog\')" title="إغلاق">إغلاق</a></span></h3>';
	if(mode == 'info') {
		s += msg ? msg : '';
	} else {
...
function showWindow(k, url, mode, cache, menuv) {
	...
	var fetchContent = function() {
		...
		if(parseInt(BROWSER.ie) != 6) {
			loadingst = setTimeout(function() {showDialog('', 'info', '<img src="' + IMGDIR + '/loading.gif"> يرجى الإنتظار...')}, 500);
		}
	};
...
function showError(msg) {
	...
	msg = msg.replace(p, '');
	if(msg !== '') {
		showDialog(msg, 'alert', 'رسالة خطأ', null, true, null, '', '', '', 3);
	}
}
...
function initSearchmenu(searchform, cloudSearchUrl) {
	...
	if (!!("placeholder" in document.createElement("input"))) {
		if(searchtxt.value == 'أدخل كلمات البحث') {
			searchtxt.value = '';
		}
		searchtxt.placeholder = 'أدخل كلمات البحث';
	} else {
		searchtxt.onfocus = function () {
			if(searchtxt.value == 'بحث بالكلمات الدلالية') {
				searchtxt.value = '';
		...
		searchtxt.onblur = function () {
			if(searchtxt.value == '' ) {
				searchtxt.value = 'بحث بالكلمات الدلالية';
				searchtxt.className = tclass + ' xg1';
			}
		};
	}
...
function searchFocus(obj) {
	if(obj.value == 'بحث بالكلمات الدلالية') {
		obj.value = '';
	}
...
function noticeTitleFlash() {
	if(NOTICETITLE.flashNumber < 5 || NOTICETITLE.flashNumber > 4 && !NOTICETITLE['State']) {
		document.title = (NOTICETITLE['State'] ? '[　　　]' : '[جديد]') + NOTICETITLE['oldTitle'];
...
function addFavorite(url, title) {
	try {
		...
		try {
			...
			showDialog("إضغط على زري  Ctrl + D للوضع بالمفضلة", 'notice');
		}
	}
}
...
function setHomepage(sURL) {
	if(BROWSER.ie){
		...
		showDialog("اذا كنت لا تستعمل متصفح IE قم بإدارج الصفحة يدوياً كرئيسية", 'notice');
		doane();
	}
}
...
function toggleBlind(dom) {
	if(dom) {
		if(loadUserdata('is_blindman')) {
			...
			dom.title = 'فتح المساعدة';
		} else {
			saveUserdata('is_blindman', '1');
			dom.title = 'إغلاق المساعدة';
		}
	}
}
