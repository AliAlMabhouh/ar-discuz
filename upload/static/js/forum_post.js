...
var STATUSMSG = {
	'-1' : 'خطأ داخلي',
	'0' : 'نجح الرفع',
	'1' : 'هذا النوع من الملفات غير مدعوم',
	'2' : 'حجم المرفقات تجاوز الحد الاقصى',
	'3' : 'مجموعتك تجاوزت الحد الاقصى من المرفقات',
	'4' : 'هذا النوع من الملفات غير مدعوم',
	'5' : 'لقد تجاوزت الحد الاقصى من حجم الملفات من هذا النوع',
	'6' : 'لا يمكنك رفع ملفات أكثر اليوم',
	'7' : 'حدد ملفات الصور(' + imgexts + ')',
	'8' : 'لا يمكن حفظ ملف المرفق',
	'9' : 'ملف غير صالح',
	'10' : 'عملية غير صالحة',
	'11' : 'لا يمكنك رفع ملفات كبيرة اليوم'
};
...
/*vot*/	} else if(theform.subject.value.length > 80) {
	...
	if(in_array($('postsubmit').name, ['topicsubmit', 'editsubmit'])) {
		if(theform.typeid && (theform.typeid.options && theform.typeid.options[theform.typeid.selectedIndex].value == 0) && typerequired) {
			showError('حدد فئة لمعلومات الموضوع');
			return false;
		}
	...
	if(!disablepostctrl && !sortid && !special && ((postminchars != 0 && mb_strlen(message) < postminchars) || (postmaxchars != 0 && mb_strlen(message) > postmaxchars))) {
		showError('طول مشاركاتك لا تلبي المتطلبات. \n\nالطول الحالي: ' + mb_strlen(message) + ' حرف\nالحد الأقصى: ' + postminchars + ' الى ' + postmaxchars + ' حرف');
		return false;
	}
	if(UPLOADSTATUS == 0) {
		if(!confirm('هناك مرفقات قيد الإنتظار, هل تريد تجاهلهم?')) {
			return false;
		}
	} else if(UPLOADSTATUS == 1) {
		showDialog('هناك بعض المرفقات قيد الرفع, الرجاء الإنتظار. سيتم نشر الموضوع تلقائي بعد إنتهاء الرفع...', 'notice');
		AUTOPOST = 1;
		return false;
	}
	if(isfirstpost && $('adddynamic') != null && $('adddynamic').checked && $('postsave') != null && isNaN(parseInt($('postsave').value)) && ($('readperm') != null && $('readperm').value || $('price') != null && $('price').value)) {
		if(confirm('هل ترغب بالتغريد لمتابعيك؟') == false) {
			return false;
		}
	}
...
function uploadNextAttach() {
	...
	var sizelimit = '';
	if(arr[4] == 'ban') {
		sizelimit = '(هذا النوع مرفوض)';
	} else if(arr[4] == 'perday') {
		sizelimit = '(يجب أن يكون الملف الواحد ليس أكبر من  ' + arr[5] + ' حرف)';
	} else if(arr[4] > 0) {
		sizelimit = '(يجب أن يكون الملف الواحد ليس أكبر من ' + arr[4] + ' حرف)';
	}
...
function uploadAttach(curId, statusid, prefix, sizelimit) {
	...
	if(curId > 0) {
		...
		if(nextId == curId || in_array(statusid, [6, 8])) {
			...
			if(UPLOADFAILED > 0) {
				showDialog('تم الرفع,نجح ' + UPLOADCOMPLETE + ' ملف，وفشل ' + UPLOADFAILED + ' ملف:' + FAILEDATTACHS);
				FAILEDATTACHS = '';
			}
			UPLOADSTATUS = 2;
			for(var i = 0; i < AID[prefix ? 1 : 0] - 1; i++) {
			...
			if(AUTOPOST) {
				...
				showDialog('تم الإنتهاء من الرفع！', 'right', null, null, 0, null, null, null, null, 3);
			}
	...
	$(prefix + 'cpdel_' + nextId).innerHTML = '<img src="' + IMGDIR + '/loading.gif" alt="يرجى الإنتظار..." />';
	UPLOADSTATUS = 1;
...
function insertAttach(prefix, id) {
	...
	if(extensions != '' && (re.exec(extensions) == null || ext == '')) {
		...
		showError('عذراً, الملف غير مدعوم.');
		return;
	}
	if(prefix == 'img' && imgexts.indexOf(ext) == -1) {
		reAddAttach(prefix, id);
		showError('حدد ملفات الصور(' + imgexts + ')');
		return;
	}

	$(prefix + 'cpdel_' + id).innerHTML = '<a href="javascript:;" class="d" onclick="reAddAttach(\'' + prefix + '\', ' + id + ')">حذف</a>';
...
function updateAttach(aid) {
	...
	if(!objupdate.innerHTML) {
		...
		objupdate.innerHTML = '<input type="file" name="attachupdate[paid' + aid + ']"><a href="javascript:;" onclick="updateAttach(' + aid + ')">إلغاء</a>';
	} else {
		obj.style.display = '';
		objupdate.innerHTML = '';
	}
...
function updateattachnum(type) {
	...
	if(num) {
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = 'يحتوي ' + num + (type == 'image' ? '  صورة مرفقة ' : '  ملفات تم إرفاقها');
		}
		...
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = type == 'image' ? ' صورة مرفقة ' : 'المرفقات';
		}
		if($(editorid + '_' + type + 'n')) {
...
function updateDownImageList(msg) {
	...
	if(msg == '') {
		showError('عذراً, لا يمكن الارفاق بوصلة');
	} else {
		...
		switchImagebutton('imgattachlist');$('imgattach_notice').style.display = '';
		showDialog('تم إرفاق المرفقات عن طريق وصلة بنجاح', 'right', null, null, 0, null, null, null, null, 3);
	}
}
...
function uploadWindowload() {
	...
	if(arr[0] == 'DISCUZUPLOAD' && arr[2] == 0) {
		...
		if(arr[7] == 'ban') {
			...
			sizelimit = '(ليس أكبر من  ' + arr[8] + ' حرف)';
		} else if(arr[7] > 0) {
			sizelimit = '(ليس أكبر من  ' + arr[7] + ' حرف)';
		}
...
function addpolloption() {
	if(curoptions < maxoptions) {
		...
		$('polloption_new').outerHTML = '<span>وقد بلغ الحد الأقصى لعدد الأصوات : '+maxoptions+'</span>';
	}
}
...
function attachoption(type, op) {
	if(!op) {
		...
		showDialog('<div id="unusedwin" class="c altw" style="overflow:auto;height:100px;">' + $('unusedlist_' + type).innerHTML + '</div>' +
			'<p class="o pns"><span class="z xg1"><label for="unusedwinchkall"><input id="unusedwinchkall" type="checkbox" onclick="attachoption(\'' + type + '\', 3)" checked="checked" />حدد الكل</label></span>' +
			'<button onclick="attachoption(\'' + type + '\', 1);hideMenu(\'fwin_dialog\', \'dialog\')" class="pn pnc"><strong>موافق</strong></button></p>', 'info', 'غير المستغلة' + (type == 'attach' ? 'المرفقات' : 'الصور'));
	} else if(op == 3) {
...
function getreplycredit() {
	...
	if(real_reply_credit > userextcredit) {
		$('replycredit').innerHTML = '<b class="xi1">مشاركات مكافئة النقاط أكبر مما يجب('+real_reply_credit+')</b>';
	} else {
		if(have_replycredit > 0 && real_reply_credit < 0) {
			$('replycredit').innerHTML = "<font class='xi1'>عودة"+Math.abs(real_reply_credit)+"</font>";
		} else {
...
function deleteThread() {
	if(confirm('هل أنت متأكد من أنك تريد حذف هذه المشاركة ؟') != 0){
		$('delete').value = '1';
		$('postform').submit();
	}
}
