...
function block_delete_item(bid, itemid, itemtype, itemfrom, from) {
	var msg = itemtype==1 ? 'هل أنت متأكد من انك تريد حذف هذه البيانات ؟' : 'هل أنت متأكد من تجاهل هذه البيانات ؟';
	if(confirm(msg)) {
...
function recommenditem_byblock(bid, id, idtype) {
	...
	if(editarea) {
		...
		if(!$('recommendback')) {
			...
			back.innerHTML = '<em id="recommendback" onclick="recommenditem_back()" class="cur1">&nbsp;&nbsp;&laquo;;رجوع</em>';
...
function toggleSettingShow() {
	if(!$('tbody_setting').style.display) {
		...
		$('a_setting_show').innerHTML = 'إظهار الإعدادات';
	} else {
		$('tbody_setting').style.display = '';
		$('a_setting_show').innerHTML = 'إخفاء الإعدادات';
	}
	doane();
}
function switchSetting() {
	...
	if(checked) {
		$('tbody_setting').style.display = 'none';
		$('a_setting_show').innerHTML = 'توسيع الإعدادات';
	} else {
		$('tbody_setting').style.display = '';
		$('a_setting_show').innerHTML = 'إخفاء الإعدادات';
	}
...
function checkblockname(form) {
	if(!(trim(form.name.value) > '')) {
		showDialog('لا يمكن ترك إسم الكتلة فارغ', 'error', null, function(){form.name.focus();});
		return false;
	}
	if(form.summary && form.summary.value) {
		...
		if(tag) {
			showBlockSummary();
			form.summary.focus();
			showDialog('محتوى مخصص غير صالح!كود  HTML : '+tag+'  - الكلمات الدلالية غير متطابقة', 'error', null, function(){form.summary.select();});
			return false;
		}
	}
		...
		for(var i = 0; i < tags.length; i++) {
			...
			if((!preTag && endTag) || (preTag && !endTag) || preTag.length != endTag.length) {
				if(returnValue) {
					...
					showDialog(' كود HTML : '+tag+'  - الكلمات الدلالية غير متطابقة', 'error', null, fn, true, fn);
					return false;
				}
			}
		}
...
function blockconver(ele,bid) {
	if(ele && bid) {
		if(confirm('هل أنت متأكد من ذلك ؟ '+ele.options[0].innerHTML+' إلى '+ele.options[ele.selectedIndex].innerHTML)) {
...
function check_itemdata_lentgh(form) {
	if(form.title && (!strLenCalc(form.title, "titlechk", form.title.getAttribute('_maxlength')) || !form.title.value)) {
		form.title.focus();
		showDialog('طول العنوان غير صالح', 'error', null, function(){form.title.select();});
		return false;
	}
	if(form.summary && !strLenCalc(form.summary, "summarychk", form.summary.getAttribute('_maxlength'))) {
		form.summary.focus();
		showDialog('طول الملخص غير صالح', 'error', null, function(){form.summary.select();});
		return false;
	}
	return true;
}