...
function removeindexheats() {
	return confirm('هل أنت متأكد من أنك تريد إزالة هذا الموضوع من المواضيع النشطة ?');
}
...
function fastpostvalidate(theform, noajaxpost) {
	...
	if(theform.message.value == '' || theform.subject.value == '') {
		s = 'عذراً , ربما لم تدخل عنوان الموضوع أو المحتوى .';
		theform.message.focus();
//vot	} else if(mb_strlen(theform.subject.value) > 80) {
/*vot*/	} else if(theform.subject.value.length > 80 ) {
		s = 'طول العنوان يزيد عن 150 حرف';
		theform.subject.focus();
	}
	if(!disablepostctrl && ((postminchars != 0 && mb_strlen(theform.message.value) < postminchars) || (postmaxchars != 0 && mb_strlen(theform.message.value) > postmaxchars))) {
		s = 'طول المحتوى لا يتطابق مع المطلوب.\n\n: ' + mb_strlen(theform.message.value) + ' ' + 'حرف\n الحد الاقصى: ' + postminchars + ' إلى ' + postmaxchars + ' حرف';
	}
	if(s) {
		showError(s);
		doane();
...
function loadData(quiet, formobj) {
	...
	if(in_array((data = trim(data)), ['', 'null', 'false', null, false])) {
		if(!quiet) {
			showDialog('لا يوجد بيانات ليتم استعادتها!', 'info');
		}
		return;
	}

	if(!quiet && !confirm('المحتوى الحالي سيتم إستبداله, هل أنت متأكد أنك ترغب في إستعادة البيانات؟')) {
		return;
	}
...
function checkForumnew(fid, lasttime) {
	...
	x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			...
			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">الموضوع يحتوي على ردود جديدة, إضغط للعرض', 'colspan': colspan }}};
...
function showtime() {
	for(i=0; i<=DTimers.length; i++) {
		if(DItemIDs[i]) {
			...
			if(timer_day > 0) {
				timestr += timer_day + 'يوم';
			}
			if(timer_hour > 0) {
				timestr += timer_hour + 'ساعة'
			}
			if(timer_minute > 0) {
				timestr += timer_minute + 'دقيقة'
			}
			if(timer_second > 0) {
				timestr += timer_second + 'ثانية'
			}
