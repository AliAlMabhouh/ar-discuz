...
function initEditor() {
	...
	for(var i = 0; i < buttons.length; i++) {
		if(buttons[i].id.indexOf(editorid + '_') != -1) {
			...
			if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'fullswitcher') {
				buttons[i].innerHTML = !editorisfull ? 'كامل' : 'بسيط';
				buttons[i].onmouseover = function(e) {setEditorTip(editorisfull ? 'إستعادة حجم المحرر' : 'كامل');};
				buttons[i].onclick = function(e) {editorfull();doane();}
			} else if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'simple') {
				buttons[i].innerHTML = !simplodemode ? 'متقدم' : 'بسيط';
	...
	if($(editorid + '_fullswitcher') && BROWSER.ie && BROWSER.ie < 7) {
		$(editorid + '_fullswitcher').onclick = function () {
			showDialog('المتصفح الخاص بك لا يدعم هذه الخاصية, قم بتحديث متصفحك', 'notice', 'نصائح');
		};
...
function savedataTime() {
	if(!autosave) {
		$(editorid + '_svdsecond').innerHTML = '<a title="إضغط هنا لتفعيل الحفظ التلقائي" href="javascript:;" onclick="setAutosave()">تفعيل الحفظ التلقائي</a> ';
		return;
	}
	if(!savedatac) {
		savedatac = savedataInterval;
		...
		h = h < 10 ? '0' + h : h;
		m = m < 10 ? '0' + m : m;
		setEditorTip(' البيانات حفظت في  ' + h + ':' + m + ' الحفظ التلقائي معطل');
	}
	$(editorid + '_svdsecond').innerHTML = '<a title="تعطيل الحفظ التلقائي" href="javascript:;" onclick="setAutosave()">' + savedatac + '  ثانية قبل الحفظ</a> ';
	savedatac -= 10;
}

function setAutosave() {
	autosave = !autosave;
	setEditorTip(autosave ? 'الحفظ التلقائي مفعل' : 'الحفظ التلقائي معطل');
...
function editorfull(op) {
	...
	$(editorid + '_fullswitcher').innerHTML = editorisfull ? 'رجوع' : 'كامل';
	initesbar();
}
...
function editorsimple() {
	if($(editorid + '_body').className == 'edt') {
		v = 'none';
		$(editorid + '_simple').innerHTML = 'متقدم';
		...
		v = '';
		$(editorid + '_simple').innerHTML = 'بسيط';
...
function checklength(theform) {
	...
	showDialog('الطول الحالي: ' + mb_strlen(message) + ' حرف, ' + (postmaxchars != 0 ? 'الحد الأقصى : ' + postminchars + ' إلى ' + postmaxchars + ' حرف' : ''), 'notice', 'فحص الطول');
}
	...
	if(wysiwyg) {
		...
		if(initialized && !(BROWSER.firefox && BROWSER.firefox >= '3' || BROWSER.opera)) {
			...
			text = '<!DOCTYPE html PUBLIC "-/' + '/W3C/' + '/DTD XHTML 1.0 Transitional/' + '/EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">' +
				'<html dir="rtl"><head id="editorheader"><meta http-equiv="Content-Type" content="text/html; charset=' + charset + '" />' +
...
function discuzcode(cmd, arg) {
	...
	if(in_array(cmd, ['sml', 'inserthorizontalrule', 'url', 'quote', 'code', 'free', 'hide', 'aud', 'vid', 'fls', 'beginning', 'attach', 'image', 'pasteword', 'index', 'postbg', 'password']) || typeof EXTRAFUNC['showEditorMenu'][cmd] != 'undefined' || cmd == 'tbl' || in_array(cmd, ['fontname', 'fontsize', 'forecolor', 'backcolor']) && !arg) {
		...
		if(txt = getSel()) {
			...
			while(listvalue = prompt('ادخل قائمة العناصر.\r\nاتركها فارغة, أو إضغط زر إلغاء.', '')) {
				if(BROWSER.opera > 8) {
					listvalue = '\n' + '[*]' + listvalue;
		...
		loadData();
		setEditorTip('تمت إستعادة البيانات');
	} else if(cmd == 'svd') {
		saveData();
		setEditorTip('البيانات حُفِظت');
	} else if(cmd == 'chck') {
		...
		showDialog('<div id="remotedowninfo"><p class="mbn">جاري تحميل المرفقات عن بعد، يرجى الانتظار ...</p><p><img src="' + STATICURL + 'image/common/uploading.gif" alt="" /></p></div>', 'notice', '', null, 1);
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'url':
				str = 'أدخل الوصلة , من فضلك :<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" />'+
					(selection ? '' : '<br />أدخل النص المرتبط:<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="px" />');
				break;
			case 'forecolor':
			...
			case 'postbg':
				...
				break;
			case 'password':
				str = '<p class="pbn">أدخل كلمة المرور: <input type="text" id="' + ctrlid + '_param_1" size="10" value="" class="px" /></p>';
				break;
			case 'code':
				if(wysiwyg) {
			...
			case 'free':
				...
				var lang = {'quote' : 'إدراج اقتباس', 'code' : 'إدراج كود', 'hide' : 'إخفاء المحتوى', 'free' : 'اذا لم تدخل سعر المشاركة, ستعرض مجاناً,'};
				str += lang[tag] + ':<br /><textarea id="' + ctrlid + '_param_1" style="width: 98%" cols="50" rows="5" class="txtarea"></textarea>' +
					(tag == 'hide' ? '<br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" class="pc" checked="checked" />إظهار عندما يرد العضو على هذا الموضوع</label><br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" class="pc" />إظهار عندما تكون نقاط العضو أكثر من</label> <input type="text" size="3" id="' + ctrlid + '_param_2" class="px pxs" /> تظهر فقط في<br /><br /><label>عدد الأيام:</label> <input type="text" size="3" id="' + ctrlid + '_param_3" class="px pxs" /> <br />يتم عرض النص تلقائيا بعد إنتهاء الأيام' : '');
				break;
			case 'tbl':
				str = '<p class="pbn">صفوف الجدول: <input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="px" /> &nbsp; أعمدة الجدول: <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="px" /></p><p class="pbn">عرض الجدول: : <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="px" /> &nbsp; لون الخلفية: <input type="text" id="' + ctrlid + '_param_4" size="2" class="px" onclick="showColorBox(this.id, 2)" /></p><p class="xg2 pbn" style="cursor:pointer" onclick="showDialog($(\'tbltips_msg\').innerHTML, \'notice\', \'نصائح\', null, 0)"><img id="tbltips" title="نصائح" class="vm" src="' + IMGDIR + '/info_small.gif"> ملاحظات بسيطة</p>';
				str += '<div id="tbltips_msg" style="display: none">“[tr=اللون]” لون الجدول<br />“[td=العرض]” عرض الصفوف<br />“[td=Column_Span,Row_Span,Width]” Define the Row/Column Span and Width<br /><br />Fast writing table example：<div class=\'xs0\' style=\'margin:0 5px\'>[table]<br />Name:|Discuz!<br />Version:|X1<br />[/table]</div>With "|”Separated each column in the table if“|”Use“\\|”Instead, wrap use“\\n”Instead.</div>';
				break;
			case 'aud':
				str = '<p class="pbn">أدخل وصلة ملف الصوت:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="xg2 pbn">يدعم wma, mp3, ogg, ra, rm, وصيغ أخرى<br />مثل: http://server/audio.wma</p>';
				break;
			case 'vid':
				str = '<p class="pbn">أدخل وصلة ملف الفيديو:</p><p class="pbn"><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 220px;" class="px" /></p><p class="pbn">العرض: <input id="' + ctrlid + '_param_2" size="5" value="500" class="px" /> &nbsp; الطول: <input id="' + ctrlid + '_param_3" size="5" value="375" class="px" /></p><p class="xg2 pbn">يدعم Youku, potatoes, 56, 6, youtube مواقع أخرى و <br /> يدعم الصيغ wmv avi rmvb mov swf flv mp4 3gp  <br /> مثال: http://server/movie.wmv</p>';
				break;
			case 'fls':
				str = '<p class="pbn">أدخل وصلة ملف فلاش :</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn">العرض: <input id="' + ctrlid + '_param_2" size="5" value="" class="px" /> &nbsp; الطول: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /></p><p class="xg2 pbn">يدعم swf flv <br /> مثال: http://server/flash.swf</p>';
				break;
			case 'beginning':
				str = '<p class="pbn">أدخل رابط الصورة أو الفلاش</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn">الرابط الهدف:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_2" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn">واسع: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /> &nbsp; طويل: <input id="' + ctrlid + '_param_4" size="5" value="" class="px" /></p>';
				str += '<p class="pbn">زمن العرض (بالثانية): <input id="' + ctrlid + '_param_8" size="5" value="" class="px" /></p>';
				str += '<p class="pbn">شكل التأثير : </p><p class="pbn"><input id="' + ctrlid + '_param_7" type="radio" name="effect" checked />بدون &nbsp; <input id="' + ctrlid + '_param_5" type="radio" name="effect" />تلاشي &nbsp; <input id="' + ctrlid + '_param_6" type="radio" name="effect" />إظهار / إخفاء</p>';
				str += '<p class="xg2 pbn">يدعم swf flv jpg gif png<br />مثال (العرض-الزمن): طويل400~1024 واسع300~640<br />مثال: http://server/flash.swf</p>';
				break;
			case 'pasteword':
				stitle = 'لصق محتوى وورد ';
				str = '<p class="px" style="height:300px"><iframe id="' + ctrlid + '_param_1" frameborder="0" style="width:100%;height:100%" onload="this.contentWindow.document.body.style.width=\'550px\';this.contentWindow.document.body.contentEditable=true;this.contentWindow.document.body.focus();this.onload=null"></iframe></p><p class="xg2 pbn">استخدم زري (Ctrl + V) للصق من ملفات وورد</p>';
				menuwidth = 600;
				menupos = '00';
				menutype = 'win';
				break;
			case 'index':
				...
				str = '<p class="pbn">[index]<br />\n\
					[#<span class="xi1">رقم الصفحة</span>]<span class="xi1">العنوان</span> &nbsp;&nbsp;<span class="xg1">الإنتقال إللا صفحة معينة</span><br />\n\
					<span class="xi1">*</span>[#<span class="xi1">tid,pid</span>]<span class="xi1">العنوان</span> &nbsp;&nbsp;<span class="xg1">الإنتقال إلى مشاركة معينة</span><br />\n\
					[/index]<br />\n\
					<br />\n\
					<span class="xi1">رقم الصفحة</span> &nbsp;&nbsp;<span class="xg1">إستخدم [page] لإستخدام الصفحة الحالية</span><br />\n\
					<span class="xi1">tid,pid</span> &nbsp;&nbsp;<span class="xg1">المواضيع TID والمشاركات PID</span><br />\n\
					<span class="xi1">*</span> &nbsp;&nbsp;<span class="xg1">إضافة المسافة البادءة</span></p>';
				break;
			default:
				...
				if(!str) {
					...
					for(var i = 1; i <= params; i++) {
						if(i != params || !haveSel) {
							str += (promptlang[i - 1] ? promptlang[i - 1] : 'الرجاء أدخل الـ ' + i + ' المعلمات:') + '<br /><input type="text" id="' + ctrlid + '_param_' + i + '" style="width: 98%" value="" class="px" />' + (i < params ? '<br />' : '');
						}
					}
				}
				break;
		...
		if(menupos == '00') {
			...
			s = '<table width="100%" cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"></td><td class="t_r"></td></tr><tr><td class="m_l">&nbsp;&nbsp;</td><td class="m_c">'
				+ '<h3 class="flb"><em>' + stitle + '</em><span><a onclick="hideMenu(\'\', \'win\');return false;" class="flbc" href="javascript:;">إغلاق</a></span></h3><div class="c">' + str + '</div>'
				+ '<p class="o pns"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>موافق</strong></button></p>'
				+ '</td><td class="m_r"></td></tr><tr><td class="b_l"></td><td class="b_c"></td><td class="b_r"></td></tr></table>';
		} else {
			s = '<div class="p_opt cl"><span class="y" style="margin:-10px -10px 0 0"><a onclick="hideMenu();return false;" class="flbc" href="javascript:;">إغلاق</a></span><div>' + str + '</div><div class="pns mtn"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>موافق</strong></button></div></div>';
		}
		menu.innerHTML = s;
	...
	if($(ctrlid + '_submit')) $(ctrlid + '_submit').onclick = function() {
		...
		switch(tag) {
			...
			case 'vid':
				...
				ext = in_array(ext, ['mp3', 'wma', 'ra', 'rm', 'ram', 'mid', 'asx', 'wmv', 'avi', 'mpg', 'mpeg', 'rmvb', 'asf', 'mov', 'flv', 'swf', 'mp4', '3gp', 'ogg']) ? ext : 'x';
				if(ext == 'x') {
