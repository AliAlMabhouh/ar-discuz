...
var lastusername = '', lastpassword = '', lastemail = '', lastinvitecode = '', stmp = new Array(), modifypwd = false, profileTips = 'إذا كنت لا تحتاج لتغيير كلمة المرور , أتركه فارغا';
...
function checkPwdComplexity(firstObj, secondObj, modify) {
	...
	firstObj.onblur = function () {
		if(firstObj.value == '') {
			var pwmsg = !modifypwd ? 'الرجاء أدخل كلمة المرور' : profileTips;
			if(pwlength > 0) {
				pwmsg += ' , الحد الأدنى من الطول  '+pwlength+' حرف';
			}
	...
	firstObj.onkeyup = function () {
		if(pwlength == 0 || $(firstObj.id).value.length >= pwlength) {
			var passlevels = new Array('','ضعيفة','متوسطة','قوية');
			var passlevel = checkstrongpw(firstObj.id);
			errormessage(firstObj.id, '<span class="passlevel passlevel'+passlevel+'">القوة:'+passlevels[passlevel]+'</span>');
		}
	};
	secondObj.onblur = function () {
		if(secondObj.value == '') {
			errormessage(secondObj.id, !modifypwd ? 'أعد كلمة المرور' : profileTips);
		}
		checkpassword(firstObj.id, secondObj.id);
	};
}
...
function addMailEvent(mailObj) {
	...
	mailObj.onblur = function () {
		if(mailObj.value == '') {
			errormessage(mailObj.id, 'الرجاء أدخل البريد الإلكتروني');
		}
...
var emailMenuST = null, emailMenui = 0, emaildomains = ['hotmail.com', 'outlook.com', 'yahoo.com', 'gmail.com', 'Maktoob.com'];
...
function checkusername(id) {
	...
	if(username.match(/<|"/ig)) {
		errormessage(id, 'إسم المستخدم يحتوي على حروف ممنوعة');
		return;
	}
...
/*vot*/	if(unlen < 3) {
/*vot*/		errormessage(id, 'يجب ألا يكون الإسم أقل من 3 حروف');
		return;
/*vot*/	} else if(unlen > 30) {
		errormessage(id, lng['username_long']);
		return;
/*vot*/	} else if(utf8len > 128) { // Max DB Field Length in bytes
/*vot*/		errormessage(id, 'يجب أن يكون الإسم أقل من 15 حرف');
/*vot*/		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkpassword(id1, id2) {
	...
	if(pwlength > 0) {
		if($(id1).value.length < pwlength) {
			errormessage(id1, 'كلمة المرور قصيرة جداً , يجب ألا تكون أقل من  '+pwlength+' حرف');
			return;
		}
	}
	if(strongpw) {
		...
		for(var i in strongpw) {
			...
			if(strongpw[i] === 2 && !$(id1).value.match(/[a-z]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = ' أحرف صغيرة ';
				j++;
			}
			if(strongpw[i] === 3 && !$(id1).value.match(/[A-Z]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = ' أحرف كبيرة';
				j++;
			}
			if(strongpw[i] === 4 && !$(id1).value.match(/[^A-Za-z0-9]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'رموز خاصة';
				j++;
			}
		}
		if(strongpw_error) {
			errormessage(id1, 'كلمة مرور ضعيفة يجب ان تحتوي على '+strongpw_str.join('，'));
			return;
		}
	}
	errormessage(id2);
	if($(id1).value != $(id2).value) {
		errormessage(id2, 'كلمتا المرور غير متطابقتين');
	} else {
...
function checkemail(id) {
	...
	if(email.match(/<|"/ig)) {
		errormessage(id, 'البريد يحتوي على أحرف ممنوعة');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkinvite() {
	...
	if(invitecode.match(/<|"/ig)) {
		errormessage('invitecode', 'كود الدعوة');
		return;
	}
	var x = new Ajax();
