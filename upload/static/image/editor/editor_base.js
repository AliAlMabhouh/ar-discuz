...
function renewContent() {
	...
	if(window.confirm('هل أنت متأكد أنك تريد استعادة آخر حفظ؟')) {
		var data = loadUserdata('home');
		if(in_array((data = trim(data)), ['', 'null', 'false', null, false])) {
			parent.showDialog('لم يتم إستعادة أي بيانات');
			return;
		}
...
function format(type, para){
	...
	if(!gIsIE){
		switch(type){
			case "Cut":
				sAlert = "الإعدادات الأمنية للمتصفح لا تسمح محرر لتنفيذ العملية , إستخدم (Ctrl+X) للقص";
				break;
			case "Copy":
				sAlert = "الإعدادات الأمنية للمتصفح لا تسمح محرر لتنفيذ العملية , إستخدم (Ctrl+C) للنسخ";
				break;
			case "Paste":
				sAlert = "الإعدادات الأمنية للمتصفح لا تسمح محرر لتنفيذ العملية , إستخدم (Ctrl+V) للصق";
				break;
		}
	}
	if(sAlert != ""){
...
function fDisplayElement(element,displayValue) {
	if(gIEVer<=5.01 && gIsIE){
		alert('يدعم فقط IE 5.01 أو أحدث');
		return;
	}
	fHideMenu();
...
function changeEditType(flag, ev){
	...
	try{
		...
		if(flag){
			...
			ev = ev || event;
			if(ev){
				if(window.confirm("قد تفقد التنسيق , هل أنت متأكد من أنك تريد المتابعة؟ ")){
