 ...
 *      International (Multi-Lingual) by Valery Votintsev at sources.ru
 */
...
/*vot*/			.'<a href="'.$theurl.'?step=prepare'.($_GET['from'] ? '&from='.rawurlencode($_GET['from']) : '').'">'.lang('update','update_start').'</a>');
	}
...
/*vot*/		$msg = lang('update','upgrade_wait').'<br><br>'.$list.base64_decode($_GET['sendsql']);
...
/*vot*/			$sql[] = "ALTER TABLE ".DB::table($post_tablename)." CHANGE `pid` `pid` INT(11) UNSIGNED NOT NULL, CHANGE `replycredit` `replycredit` int(11) NOT NULL default '0', CHANGE `status` `status` int(11) NOT NULL default '0', ADD UNIQUE KEY pid (pid), DROP PRIMARY KEY, ADD `position` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, ADD PRIMARY KEY(`tid`, `position`), DROP INDEX authorid, ADD INDEX authorid (authorid,invisible)";
			if(!$_GET['waitingdb']) {
...
/*vot*/			$article_tagsnew = addslashes(serialize(lang('update','article_tags')));
...
/*vot*/		echo "<p><input type=\"submit\" name=\"delsubmit\" value=\"".lang('update','submit_delete')."\"></p><p>".lang('update','ignore_unused')."<br><a href=\"$theurl?step=style\">".lang('update','next_step_directly')."</a></p>";
	}
	echo '</form>';
...
/*vot*/		show_msg(lang('update','default_style_restore_sure')."<br /><br /><a href=\"$theurl?step=style&confirm=yes\">[ ".lang('update','yes')." ]</a>&nbsp;&nbsp;<a href=\"$theurl?step=cache\">[ ".lang('update','no')." ]</a>", '');
	}
...
/*vot*/		$message = "<a href=\"$url_forward\">$message (".lang('update','jump_to').")</a><script>setTimeout(\"window.location.href ='$url_forward';\", $time);</script>";
	}
...
/*vot*/	print<<<END
	...
	<br />Arabic By Discuz.TaxiWeb.Org</div>
	</div>
	<br>
	</body>
	</html>
